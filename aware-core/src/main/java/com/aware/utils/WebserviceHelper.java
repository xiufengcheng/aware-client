...
public class WebserviceHelper extends IntentService {
	...
	public static String ACTION_AWARE_BATTERY_STATUS=  "ACTION_BATTERY_STATUS";
...
/*	public void onReceive(Context context, Intent intent) {
		ACTION_AWARE_BATTERY_STATUS = intent.getAction();

		Log.i("Receiver", "Broadcast battery received: " + ACTION_AWARE_BATTERY_STATUS);

		if(ACTION_AWARE_BATTERY_STATUS.equals("ACTION_AWARE_BATTERY_CHARGING")){
	...
	protected void onHandleIntent(Intent intent) {
		...
		if (isCharging==true) {
            if (intent.getAction().equals(ACTION_AWARE_WEBSERVICE_SYNC_TABLE)) {
                ...
                //Check if we are supposed to s-ync over WiFi only
                if (Aware.getSetting(getApplicationContext(), Aware_Preferences.WEBSERVICE_WIFI_ONLY).equals("true")) //&& isCharging == true)
                {
                    // check for battery status
                    //	if (Battery.ACTION_AWARE_BATTERY_CHARGING.equals(true)){
